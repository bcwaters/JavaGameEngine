...
class EnvironmentPhysics //this class will add vectors of objects and detect collisions
{
...

    {
        ...
        speedLimit = new Vector(20, 20); // I will make a new environment physics record which hold info like speed limit gravity etc to pass in a constructor
    }

    public void applyVectors(GameObject selectedObject, GameObject comparedObject) //This Method will be moved out to Collision detection
    {
    ...
    public void applySpeedLimit() {
        ...
        for (int i = 0; gameObjects[i].getObjectType() != gameObjects[i].NULLOBJECT; i++) {
            ...
            if (gameObjects[i].withinVectorBounds(speedLimit)) //if the speedLimit is broken change object
            {
                ...
                addGameEvent(new GameEvent(eventVector, i, event.LIMITEVENT)); //adds an event to the array that is being returned. This event will contain the objects new vector
            }
    ...
    public GameEvent[] returnChanges() //this returns an array of gameEvents to be processed
    {
